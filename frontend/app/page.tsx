  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col h-full justify-between transform-style-3d translate-z-20">
        <div>
          <div className="bg-white/20 w-16 h-16 rounded-full flex items-center justify-center backdrop-blur-md mb-6 shadow-lg border border-white/30">
            <Icon size={32} />
          </div>
    ...
    try {
      ...
      const res = await fetch('http://localhost:5001/api/payments/create-order', {
        ...
        body: JSON.stringify({
          ...
          seriesId: series.id,
          userId: user.uid // Ensure AuthContext provides uid
        })
      });
      ...
      // 2. Open Razorpay
      const options = {
        key: "rzp_test_1234567890", // Replace with env variable in real app
        amount: orderData.amount,
        currency: orderData.currency,
        name: "ExamPortal",
        description: `Purchase ${series.title}`,
        ...
        handler: async function (response: any) {
          ...
          if (verifyData.success) {
            alert('Payment Successful! You can now access the series.');
      ...
      <div ref={vantaRef} className="min-h-screen w-full relative overflow-hidden">
        ...
        <div className="relative z-10 min-h-screen flex flex-col">
          ...
          <main className="flex-grow flex flex-col justify-center items-center px-4 pt-24 pb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 max-w-7xl w-full mx-auto perspective-1000 mb-20">
          ...
          <footer className="w-full text-center py-6 text-slate-600 bg-white/10 backdrop-blur-lg">
            <p className="flex items-center justify-center gap-2 font-medium">
              <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
              System Operational • Capable of handling 10k+ concurrent users
            </p>
          </footer>
        </div>
      </div>
