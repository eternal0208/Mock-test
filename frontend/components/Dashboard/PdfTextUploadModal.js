...
const PdfTextUploadModal = ({ onUpload, onClose }) => {
    ...
    const captureSelection = async () => {
        if (!selection || !pdf) {
            console.warn("Capture aborted: selection or PDF missing", { selection: !!selection, pdf: !!pdf });
            return;
        }

        setIsCapturing(true);
        console.log("Extracting text for slot:", activeSlot);
        try {
            const page = await pdf.getPage(currentPage);
            ...
            if (activeSlot.startsWith('opt') && activeSlot !== 'optD' && containerRef.current) {
                setSelection(prev => {
                    if (!prev) return null;
    ...
    const handleAddToQueue = () => {
        if (!currentQuestionData.text.trim()) {
            alert("Please select the question area to extract text or type the question text.");
            return;
        }
                ...
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-8 transform transition-all">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
                            <Type className="text-indigo-600" /> PDF Text to Mock Test
                        </h3>
                    ...
                    <div className="border-3 border-dashed border-indigo-100 rounded-xl p-10 text-center hover:border-indigo-300 transition-colors group bg-indigo-50/30">
                        ...
                        <label className="cursor-pointer">
                            <span className="bg-indigo-600 text-white px-8 py-3 rounded-lg font-bold hover:bg-indigo-700 transition shadow-lg shadow-indigo-200 inline-block">
                                Choose PDF for Text Extraction
                            </span>
                                ...
                                >
                                    ...
                                    >
                                        <button
                                            ...
                                            className={`bg-indigo-500 text-white px-4 py-2 rounded-full text-xs font-black shadow-2xl shadow-indigo-500/40 flex items-center gap-1.5 whitespace-nowrap ring-2 ring-black/20 ${(!pdf || isCapturing) ? 'opacity-50 cursor-not-allowed' : 'hover:bg-indigo-400 active:scale-95'} transition-all`}
                                        >
                                            <Type size={14} /> {isCapturing ? 'Extracting…' : `Extract Text → ${activeSlot === 'question' ? 'Q' : activeSlot === 'solution' ? 'S' : activeSlot.replace('opt', '')}`}
                                        </button>
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
                ...
                <div className="flex-1 p-4 space-y-3 overflow-y-auto">
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        ...
                        <div className="p-3 space-y-1.5">
                            ...
                            {[{ id: 'question', label: 'Question', key: 'Q' }].map(slot => (
                                <div
                                    key={slot.id}
                                    onClick={() => setActiveSlot(slot.id)}
                                ...
                                >
                                    <div className="flex items-center justify-between mb-1.5">
                                        ...
                                        {currentQuestionData.text.trim() ? <CheckCircle size={14} className="text-emerald-500" /> : <div className="w-3 h-3 rounded-full border-2 border-gray-300" />}
                                    </div>
                                    <div className="mt-2" onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()}>
                                        <RichMathEditor
                                            ...
                                            placeholder="Type question text here or extract from PDF..."
                            ...
                            {currentQuestionData.type !== 'integer' && (
                                <div className="grid grid-cols-2 gap-1.5">
                                    {[
                                        { id: 'optA', label: 'Option A', key: '1' },
                                        { id: 'optB', label: 'Option B', key: '2' },
                                        { id: 'optC', label: 'Option C', key: '3' },
                                        { id: 'optD', label: 'Option D', key: '4' },
                                    ].map(slot => {
                                        ...
                                        const isCorrect = isCorrectMCQ || isCorrectMSQ;
                                        const optValue = currentQuestionData.options[['optA', 'optB', 'optC', 'optD'].indexOf(slot.id)] || '';
                                            ...
                                            >
                                                ...
                                                <div className="mt-1.5" onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()}>
                                                    <RichMathEditor
                                                        ...
                                                        placeholder={`Extract or Type ${optLetter}`}
                                                ...
                                                <button
                                                    ...
                                                    className={`w-full mt-2 py-1 rounded text-[9px] font-bold uppercase tracking-wider transition-all border ${isCorrect ? 'bg-emerald-500 text-white border-emerald-600 shadow' : 'bg-white text-gray-400 border-gray-200 hover:bg-gray-50 hover:text-emerald-600'}`}
                                                >
                            ...
                            >
                                <div className="flex items-center justify-between mb-1.5">
                                    ...
                                    {currentQuestionData.solution?.trim() ? (
                                ...
                                <div className="mt-2" onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()}>
                                    <RichMathEditor
                                        ...
                                        placeholder="Type solution/explanation here or extract from PDF..."
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        ...
                        >
                            <div className="flex items-center gap-2">
                                ...
                                {(currentQuestionData.optionsLayout === 'grid') && (
